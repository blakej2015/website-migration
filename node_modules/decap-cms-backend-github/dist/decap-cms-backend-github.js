...
`,ea=Ro`
  ...
  ${Io}
`,ta="no-cache",ra="cache-first",na=new Qn({introspectionQueryResultData:yi()});function ia(e){return{...e,labels:e.labels.nodes,head:{ref:e.headRefName,sha:e.headRefOid,repo:{fork:e.repository.isFork}},base:{ref:e.baseRefName,sha:e.baseRefOid}}}class oa extends it{constructor(e){super(e),this.client=this.getApolloClient()}getApolloClient(){const e=(r=(e,{headers:t})=>({headers:{"Content-Type":"application/json; charset=utf-8",...t,authorization:this.token?`${this.tokenKeyword} ${this.token}`:""}}),new qr(function(e,t){var n=function(e,t){var r={};for(var n in e)Object.prototype.hasOwnProperty.call(e,n)&&t.indexOf(n)<0&&(r[n]=e[n]);if(null!=e&&"function"==typeof Object.getOwnPropertySymbols){var i=0;for(n=Object.getOwnPropertySymbols(e);i<n.length;i++)t.indexOf(n[i])<0&&Object.prototype.propertyIsEnumerable.call(e,n[i])&&(r[n[i]]=e[n[i]])}return r}(e,[]);return new Er(function(i){var o;return Promise.resolve(n).then(function(t){return r(t,e.getContext())}).then(e.setContext).then(function(){o=t(e).subscribe({next:i.next.bind(i),error:i.error.bind(i),complete:i.complete.bind(i)})}).catch(i.error.bind(i)),function(){o&&o.unsubscribe()}})})),t=di({uri:`${this.apiRoot}/graphql`});var r;return new en({link:e.concat(t),cache:new ai({fragmentMatcher:na}),defaultOptions:{watchQuery:{fetchPolicy:ta,errorPolicy:"ignore"},query:{fetchPolicy:ta,errorPolicy:"all"}}})}reset(){return this.client.resetStore()}async getRepository(e,t){const{data:r}=await this.query({query:Lo,variables:{owner:e,name:t},fetchPolicy:ra});return r.repository}query(e){return this.client.query(e).catch(e=>{throw new O.APIError(e.message,500,"GitHub")})}async mutate(e){try{return await this.client.mutate(e)}catch(t){const r=t.graphQLErrors;if(Array.isArray(r)&&r.some(e=>"Ref cannot be created."===e.message)){const t=e?.variables?.createRefInput?.name||"",r=s()(t,"refs/heads/");r&&await(0,O.throwOnConflictingBranches)(r,e=>this.getBranch(e),Ye)}else if(Array.isArray(r)&&r.some(e=>new RegExp(`A ref named "refs/heads/${O.CMS_BRANCH_PREFIX}/.+?" already exists in the repository.`).test(e.message))){const t=e?.variables?.createRefInput?.name||"",r=e?.variables?.createRefInput?.oid||"",n=s()(t,"refs/heads/");if(n&&n.startsWith(`${O.CMS_BRANCH_PREFIX}/`)&&r)try{return await this.backupBranch(n),await this.deleteBranch(n),await this.client.mutate(e)}catch(e){console.log(e)}}throw new O.APIError(t.message,500,"GitHub")}}async hasWriteAccess(){const{repoOwner:e,repoName:t}=this;try{const{data:r}=await this.query({query:Ao,variables:{owner:e,name:t},fetchPolicy:ra}),{viewerPermission:n}=r.repository;return["ADMIN","MAINTAIN","WRITE"].includes(n)}catch(e){throw console.error("Problem fetching repo data from GitHub"),e}}async user(){const{data:e}=await this.query({query:Po,fetchPolicy:ra});return e.viewer}async retrieveBlobObject(e,t,r,n={}){const{data:i}=await this.query({query:Co,variables:{owner:e,name:t,expression:r},...n});if(i.repository.object){const{is_binary:e,text:t}=i.repository.object;return{isNull:!1,isBinary:e,text:t}}return{isNull:!0}}getOwnerAndNameFromRepoUrl(e){let{repoOwner:t,repoName:r}=this;return e===this.originRepoURL&&({originRepoOwner:t,originRepoName:r}=this),{owner:t,name:r}}async readFile(e,t,{branch:r=this.branch,repoURL:n=this.repoURL,parseText:i=!0}={}){return t||(t=await this.getFileSha(e,{repoURL:n,branch:r})),await(0,O.readFile)(t,()=>this.fetchBlobContent({sha:t,repoURL:n,parseText:i}),O.localForage,i)}async fetchBlobContent({sha:e,repoURL:t,parseText:r}){if(!r)return super.fetchBlobContent({sha:e,repoURL:t,parseText:r});const{owner:n,name:i}=this.getOwnerAndNameFromRepoUrl(t),{isNull:o,isBinary:a,text:s}=await this.retrieveBlobObject(n,i,e,{fetchPolicy:ra});if(o)throw new O.APIError("Not Found",404,"GitHub");return a?super.fetchBlobContent({sha:e,repoURL:t,parseText:r}):s}async getPullRequestAuthor(e){const t=e.user;return t?.name||t?.login}async getPullRequests(e,t,r){const{originRepoOwner:n,originRepoName:i}=this;let o;o=t===et.Open?["OPEN"]:t===et.Closed?["CLOSED","MERGED"]:["OPEN","CLOSED","MERGED"];const{data:a}=await this.query({query:jo,variables:{owner:n,name:i,...e?{head:e}:{},states:o}}),{pullRequests:s}=a.repository;return s.nodes.map(ia).filter(e=>e.head.ref.startsWith(`${O.CMS_BRANCH_PREFIX}/`)&&r(e))}async getOpenAuthoringBranches(){const{repoOwner:e,repoName:t}=this,{data:r}=await this.query({query:Bo,variables:{owner:e,name:t,refPrefix:`refs/heads/cms/${this.repo}/`}});return r.repository.refs.nodes.map(({name:e,prefix:t})=>({ref:`${t}${e}`}))}async getStatuses(e,t){const r=this.generateContentKey(e,t),n=(0,O.branchFromContentKey)(r),i=(await this.getBranchPullRequest(n)).head.sha,{originRepoOwner:o,originRepoName:a}=this,{data:s}=await this.query({query:No,variables:{owner:o,name:a,sha:i}});if(s.repository.object){const{status:e}=s.repository.object,{contexts:t}=e||{contexts:[]};return t}return[]}getAllFiles(e,t){return e.reduce((e,r)=>{if("tree"===r.type){const n=r.object?.entries||[];return[...e,...this.getAllFiles(n,`${t}/${r.name}`)]}return"blob"===r.type?[...e,{name:r.name,type:r.type,id:r.sha,path:`${t}/${r.name}`,size:r.blob?r.blob.size:0}]:e},[])}async listFiles(e,{repoURL:t=this.repoURL,branch:r=this.branch,depth:n=1}={}){const{owner:i,name:o}=this.getOwnerAndNameFromRepoUrl(t),a=He()(e,"/"),{data:s}=await this.query({query:qo(n),variables:{owner:i,name:o,expression:`${r}:${a}`}});return s.repository.object?this.getAllFiles(s.repository.object.entries,a):[]}getBranchQualifiedName(e){return`refs/heads/${e}`}getBranchQuery(e,t,r){return{query:Do,variables:{owner:t,name:r,qualifiedName:this.getBranchQualifiedName(e)}}}async getDefaultBranch(){const{data:e}=await this.query({...this.getBranchQuery(this.branch,this.originRepoOwner,this.originRepoName)});return e.repository.branch}async getBranch(e){const{data:t}=await this.query({...this.getBranchQuery(e,this.repoOwner,this.repoName),fetchPolicy:ra});if(!t.repository.branch)throw new O.APIError("Branch not found",404,Ye);return t.repository.branch}async patchRef(e,t,r,n={}){if("heads"!==e)return super.patchRef(e,t,r,n);const i=n.force||!1,o=await this.getBranch(t),{data:a}=await this.mutate({mutation:Vo,variables:{input:{oid:r,refId:o.id,force:i}}});return a.updateRef.branch}async deleteBranch(e){const t=await this.getBranch(e),{data:r}=await this.mutate({mutation:Ko,variables:{deleteRefInput:{refId:t.id}},update:e=>e.data.delete(ni(t))});return r.deleteRef}getPullRequestQuery(e){const{originRepoOwner:t,originRepoName:r}=this;return{query:Uo,variables:{owner:t,name:r,number:e}}}async getPullRequest(e){const{data:t}=await this.query({...this.getPullRequestQuery(e),fetchPolicy:ra}),r="OPEN"===t.repository.pullRequest.state?et.Open:et.Closed;return{...t.repository.pullRequest,state:r}}getPullRequestAndBranchQuery(e,t){const{repoOwner:r,repoName:n}=this,{originRepoOwner:i,originRepoName:o}=this;return{query:$o,variables:{owner:r,name:n,originRepoOwner:i,originRepoName:o,number:t,qualifiedName:this.getBranchQualifiedName(e)}}}async getPullRequestAndBranch(e,t){const{data:r}=await this.query({...this.getPullRequestAndBranchQuery(e,t),fetchPolicy:ra}),{repository:n,origin:i}=r;return{branch:n.branch,pullRequest:i.pullRequest}}async openPR(e){const t=await this.getPullRequest(e),{data:r}=await this.mutate({mutation:ea,variables:{reopenPullRequestInput:{pullRequestId:t.id}},update:(e,{data:t})=>{const{pullRequest:r}=t.reopenPullRequest,n={repository:{...r.repository,pullRequest:r}};e.writeQuery({...this.getPullRequestQuery(r.number),data:n})}});return r.reopenPullRequest}async closePR(e){const t=await this.getPullRequest(e),{data:r}=await this.mutate({mutation:Wo,variables:{closePullRequestInput:{pullRequestId:t.id}},update:(e,{data:t})=>{const{pullRequest:r}=t.closePullRequest,n={repository:{...r.repository,pullRequest:r}};e.writeQuery({...this.getPullRequestQuery(r.number),data:n})}});return r.closePullRequest}async deleteUnpublishedEntry(e,t){try{const r=this.generateContentKey(e,t),n=(0,O.branchFromContentKey)(r),i=await this.getBranchPullRequest(n);if(i.number!==Xe){const{branch:e,pullRequest:t}=await this.getPullRequestAndBranch(n,i.number),{data:r}=await this.mutate({mutation:Ho,variables:{deleteRefInput:{refId:e.id},closePullRequestInput:{pullRequestId:t.id}},update:r=>{r.data.delete(ni(e)),r.data.delete(ni(t))}});return r.closePullRequest}return await this.deleteBranch(n)}catch(e){const{graphQLErrors:t}=e;if(t&&t.length>0){const e=t.some(e=>"NOT_FOUND"===e.type);if(e)return}throw e}}async createPR(e,t){const[r,n]=await Promise.all([this.getRepository(this.originRepoOwner,this.originRepoName),this.useOpenAuthoring?`${(await this.user()).login}:${t}`:t]),{data:i}=await this.mutate({mutation:Yo,variables:{createPullRequestInput:{baseRefName:this.branch,body:O.DEFAULT_PR_BODY,title:e,headRefName:n,repositoryId:r.id}},update:(e,{data:t})=>{const{pullRequest:r}=t.createPullRequest,n={repository:{...r.repository,pullRequest:r}};e.writeQuery({...this.getPullRequestQuery(r.number),data:n})}}),{pullRequest:o}=i.createPullRequest;return{...o,head:{sha:o.headRefOid}}}async createBranch(e,t){const r=this.repoOwner,n=this.repoName,i=await this.getRepository(r,n),{data:o}=await this.mutate({mutation:Xo,variables:{createRefInput:{name:this.getBranchQualifiedName(e),oid:t,repositoryId:i.id}},update:(t,{data:i})=>{const{branch:o}=i.createRef,a={repository:{...o.repository,branch:o}};t.writeQuery({...this.getBranchQuery(e,r,n),data:a})}}),{branch:a}=o.createRef;return{...a,ref:`${a.prefix}${a.name}`}}async createBranchAndPullRequest(e,t,r){const n=this.originRepoOwner,i=this.originRepoName,o=await this.getRepository(n,i),{data:a}=await this.mutate({mutation:Zo,variables:{createRefInput:{name:this.getBranchQualifiedName(e),oid:t,repositoryId:o.id},createPullRequestInput:{baseRefName:this.branch,body:O.DEFAULT_PR_BODY,title:r,headRefName:e,repositoryId:o.id}},update:(t,{data:r})=>{const{branch:o}=r.createRef,{pullRequest:a}=r.createPullRequest,s={repository:{...o.repository,branch:o}},u={repository:{...a.repository,branch:o},origin:{...a.repository,pullRequest:a}};t.writeQuery({...this.getBranchQuery(e,n,i),data:s}),t.writeQuery({...this.getPullRequestAndBranchQuery(e,a.number),data:u})}}),{pullRequest:s}=a.createPullRequest;return ia(s)}async getFileSha(e,{repoURL:t=this.repoURL,branch:r=this.branch}={}){const{owner:n,name:i}=this.getOwnerAndNameFromRepoUrl(t),{data:o}=await this.query({query:Qo,variables:{owner:n,name:i,expression:`${r}:${e}`}});if(o.repository.file)return o.repository.file.sha;throw new O.APIError("Not Found",404,Ye)}}function aa(){return aa=Object.assign?Object.assign.bind():function(e){for(var t=1;t<arguments.length;t++){var r=arguments[t];for(var n in r)({}).hasOwnProperty.call(r,n)&&(e[n]=r[n])}return e},aa.apply(null,arguments)}const{fetchWithTimeout:sa}=O.unsentRequest,ua="https://www.githubstatus.com",ca=`${ua}/api/v2/components.json`,la=["API Requests","Issues, Pull Requests, Projects"],ha={GitHubBackend:class{bypassWriteAccessCheckForAppTokens=!1;constructor(e,t={}){if(this.options={proxied:!1,API:null,initialWorkflowStatus:"",...t},!this.options.proxied&&(null===e.backend.repo||void 0===e.backend.repo))throw new Error('The GitHub backend needs a "repo" in the backend configuration.');if(this.api=this.options.API||null,this.isBranchConfigured=!!e.backend.branch,this.openAuthoringEnabled=e.backend.open_authoring||!1,this.openAuthoringEnabled){if(!this.options.useWorkflow)throw new Error("backend.open_authoring is true but publish_mode is not set to editorial_workflow.");this.originRepo=e.backend.repo||""}else this.repo=this.originRepo=e.backend.repo||"";this.alwaysForkEnabled=e.backend.always_fork||!1,this.branch=e.backend.branch?.trim()||"master",this.apiRoot=e.backend.api_root||"https://api.github.com",this.token="",this.tokenKeyword="token",this.baseUrl=e.backend.base_url,this.squashMerges=e.backend.squash_merges||!1,this.cmsLabelPrefix=e.backend.cms_label_prefix||"",this.useGraphql=e.backend.use_graphql||!1,this.mediaFolder=e.media_folder,this.previewContext=e.backend.preview_context||"",this.lock=(0,O.asyncLock)()}isGitBackend(){return!0}async status(){const e=await sa(ca).then(e=>e.json()).then(e=>e.components.filter(e=>la.includes(e.name)).every(e=>"operational"===e.status)).catch(e=>(console.warn("Failed getting GitHub status",e),!0));let t=!1;return e&&(t=await(this.api?.getUser({token:this.token??""}).then(e=>!!e).catch(e=>(console.warn("Failed getting GitHub user",e),!1)))||!1),{auth:{status:t},api:{status:e,statusPage:ua}}}authComponent(){const e=e=>(0,S.jsx)(re,aa({},e,{backend:this}));return e.displayName="AuthenticationPage",e}restoreUser(e){return this.openAuthoringEnabled?this.authenticateWithFork({userData:e,getPermissionToFork:()=>!0}).then(()=>this.authenticate(e)):this.authenticate(e)}async pollUntilForkExists({repo:e,token:t}){let r=!1;for(;!r;)r=await sa(`${this.apiRoot}/repos/${e}`,{headers:{Authorization:`${this.tokenKeyword} ${t}`}}).then(()=>!0).catch(e=>e&&404===e.status?(console.log("This 404 was expected and handled appropriately."),!1):Promise.reject(e)),r||await new Promise(e=>setTimeout(e,250));return Promise.resolve()}async currentUser({token:e}){return this._currentUserPromise||(this._currentUserPromise=(async()=>{const t=await sa(`${this.apiRoot}/user`,{headers:{Authorization:`${this.tokenKeyword} ${e}`}}),r=await t.json();return{...r,name:r.name||"Unknown"}})()),this._currentUserPromise}async userIsOriginMaintainer({username:e,token:t}){const r=e||(await this.currentUser({token:t})).login;return this._userIsOriginMaintainerPromises=this._userIsOriginMaintainerPromises||{},this._userIsOriginMaintainerPromises[r]||(this._userIsOriginMaintainerPromises[r]=sa(`${this.apiRoot}/repos/${this.originRepo}/collaborators/${r}/permission`,{headers:{Authorization:`${this.tokenKeyword} ${t}`}}).then(e=>e.json()).then(({permission:e})=>"admin"===e||"write"===e)),this._userIsOriginMaintainerPromises[r]}async forkExists({token:e}){try{const t=await this.currentUser({token:e}),r=this.originRepo.split("/")[1],n=await sa(`${this.apiRoot}/repos/${t.login}/${r}`,{method:"GET",headers:{Authorization:`${this.tokenKeyword} ${e}`}}).then(e=>e.json());return!0===n.fork&&n.parent&&n.parent.full_name.toLowerCase()===this.originRepo.toLowerCase()}catch{return!1}}async authenticateWithFork({userData:e,getPermissionToFork:t}){if(!this.openAuthoringEnabled)throw new Error("Cannot authenticate with fork; Open Authoring is turned off.");const r=e.token;if(!this.alwaysForkEnabled&&await this.userIsOriginMaintainer({token:r}))return this.repo=this.originRepo,this.useOpenAuthoring=!1,Promise.resolve();const n=await this.currentUser({token:r}),i=this.originRepo.split("/")[1];if(this.repo=`${n.login}/${i}`,this.useOpenAuthoring=!0,await this.forkExists({token:r}))return sa(`${this.apiRoot}/repos/${this.repo}/merge-upstream`,{method:"POST",headers:{Authorization:`${this.tokenKeyword} ${r}`},body:JSON.stringify({branch:this.branch})});{await t();const e=await sa(`${this.apiRoot}/repos/${this.originRepo}/forks`,{method:"POST",headers:{Authorization:`${this.tokenKeyword} ${r}`}}).then(e=>e.json());return this.pollUntilForkExists({repo:e.full_name,token:r})}}async authenticate(e){if(this.token=e.token,!this.isBranchConfigured){const e=await sa(`${this.apiRoot}/repos/${this.originRepo}`,{headers:{Authorization:`token ${this.token}`}}).then(e=>e.json()).catch(()=>null);e&&e.default_branch&&(this.branch=e.default_branch)}const t=this.useGraphql?oa:it;this.api=new t({token:this.token,tokenKeyword:this.tokenKeyword,branch:this.branch,repo:this.repo,originRepo:this.originRepo,apiRoot:this.apiRoot,squashMerges:this.squashMerges,cmsLabelPrefix:this.cmsLabelPrefix,useOpenAuthoring:this.useOpenAuthoring,initialWorkflowStatus:this.options.initialWorkflowStatus,baseUrl:this.baseUrl,getUser:e=>this.currentUser(e)});const r=await this.api.user();if(!await this.api.hasWriteAccess().catch(e=>{throw e.message=k`
      ...
      `,e})&&!this.bypassWriteAccessCheckForAppTokens)throw new Error("Your GitHub user account does not have access to this repo.");return{...r,token:e.token,useOpenAuthoring:this.useOpenAuthoring}}logout(){if(this.token=null,this.api&&this.api.reset&&"function"==typeof this.api.reset)return this.api.reset()}getToken(){return Promise.resolve(this.token)}getCursorAndFiles=(e,t)=>{const r=e.length,n=Math.ceil(e.length/20),i=[];return t>1&&(i.push("prev"),i.push("first")),t<n&&(i.push("next"),i.push("last")),{cursor:O.Cursor.create({actions:i,meta:{page:t,count:r,pageSize:20,pageCount:n},data:{files:e}}),files:e.slice(20*(t-1),20*t)}};async entriesByFolder(e,t,r){const n=this.api.originRepoURL;let i;const o=await(0,O.entriesByFolder)(()=>this.api.listFiles(e,{repoURL:n,depth:r}).then(e=>{const r=e.filter(e=>(0,O.filterByExtension)(e,t)),n=this.getCursorAndFiles(r,1);return i=n.cursor,n.files}),(e,t)=>this.api.readFile(e,t,{repoURL:n}),this.api.readFileMetadata.bind(this.api),Ye);return o[O.CURSOR_COMPATIBILITY_SYMBOL]=i,o}async allEntriesByFolder(e,t,r,n){const i=this.api.originRepoURL;return await(0,O.entriesByFolder)(()=>this.api.listFiles(e,{repoURL:i,depth:r}).then(e=>e.filter(e=>(!n||n.test(e.path))&&(0,O.filterByExtension)(e,t))),(e,t)=>this.api.readFile(e,t,{repoURL:i}),this.api.readFileMetadata.bind(this.api),Ye)}entriesByFiles(e){const t=this.useOpenAuthoring?this.api.originRepoURL:this.api.repoURL;return(0,O.entriesByFiles)(e,(e,r)=>this.api.readFile(e,r,{repoURL:t}).catch(()=>""),this.api.readFileMetadata.bind(this.api),Ye)}getEntry(e){const t=this.api.originRepoURL;return this.api.readFile(e,null,{repoURL:t}).then(t=>({file:{path:e,id:null},data:t})).catch(()=>({file:{path:e,id:null},data:""}))}getMedia(e=this.mediaFolder){return this.api.listFiles(e).then(e=>e.map(({id:e,name:t,size:r,path:n})=>({id:e,name:t,size:r,displayURL:{id:e,path:n},path:n})))}async getMediaFile(e){const t=await(0,O.getMediaAsBlob)(e,null,this.api.readFile.bind(this.api)),r=(0,O.basename)(e),n=(0,O.blobToFileObj)(r,t),i=URL.createObjectURL(n);return{id:await(0,O.getBlobSHA)(t),displayURL:i,path:e,name:r,size:n.size,file:n,url:i}}getMediaDisplayURL(e){return this._mediaDisplayURLSem=this._mediaDisplayURLSem||o()(10),(0,O.getMediaDisplayURL)(e,this.api.readFile.bind(this.api),this._mediaDisplayURLSem)}persistEntry(e,t){return(0,O.runWithLock)(this.lock,()=>this.api.persistFiles(e.dataFiles,e.assets,t),"Failed to acquire persist entry lock")}async persistMedia(e,t){try{await this.api.persistFiles([],[e],t);const{sha:r,path:n,fileObj:i}=e,o=i?URL.createObjectURL(i):"";return{id:r,name:i.name,size:i.size,displayURL:o,path:s()(n,"/")}}catch(e){throw console.error(e),e}}deleteFiles(e,t){return this.api.deleteFiles(e,t)}async traverseCursor(e,t){const r=e.meta,n=e.data.get("files").toJS();let i;switch(t){case"first":default:i=this.getCursorAndFiles(n,1);break;case"last":i=this.getCursorAndFiles(n,r.get("pageCount"));break;case"next":i=this.getCursorAndFiles(n,r.get("page")+1);break;case"prev":i=this.getCursorAndFiles(n,r.get("page")-1)}return{entries:await(0,O.entriesByFiles)(i.files,(e,t)=>this.api.readFile(e,t,{repoURL:this.api.originRepoURL}).catch(()=>""),this.api.readFileMetadata.bind(this.api),Ye),cursor:i.cursor}}async loadMediaFile(e,t){const r=await(0,O.getMediaAsBlob)(t.path,t.id,(t,r,{parseText:n})=>this.api.readFile(t,r,{branch:e,parseText:n})),n=(0,O.basename)(t.path),i=(0,O.blobToFileObj)(n,r);return{id:t.id,displayURL:URL.createObjectURL(i),path:t.path,name:n,size:i.size,file:i}}async unpublishedEntries(){const e=await(0,O.unpublishedEntries)(()=>this.api.listUnpublishedBranches().then(e=>e.map(e=>(0,O.contentKeyFromBranch)(e))));return e}async unpublishedEntry({id:e,collection:t,slug:r}){if(e)return await this.api.retrieveUnpublishedEntryData(e);if(t&&r){const e=this.api.generateContentKey(t,r);return await this.api.retrieveUnpublishedEntryData(e)}throw new Error("Missing unpublished entry id or collection and slug")}getBranch(e,t){const r=this.api.generateContentKey(e,t);return(0,O.branchFromContentKey)(r)}async unpublishedEntryDataFile(e,t,r,n){const i=this.getBranch(e,t);return await this.api.readFile(r,n,{branch:i})}async unpublishedEntryMediaFile(e,t,r,n){const i=this.getBranch(e,t);return await this.loadMediaFile(i,{path:r,id:n})}async getDeployPreview(e,t){try{const r=await this.api.getStatuses(e,t),n=(0,O.getPreviewStatus)(r,this.previewContext);if(n){const{target_url:e,state:t}=n;return{url:e,status:t}}return null}catch(e){return null}}updateUnpublishedEntryStatus(e,t,r){return(0,O.runWithLock)(this.lock,()=>this.api.updateUnpublishedEntryStatus(e,t,r),"Failed to acquire update entry status lock")}deleteUnpublishedEntry(e,t){return(0,O.runWithLock)(this.lock,()=>this.api.deleteUnpublishedEntry(e,t),"Failed to acquire delete entry lock")}publishUnpublishedEntry(e,t){return(0,O.runWithLock)(this.lock,()=>this.api.publishUnpublishedEntry(e,t),"Failed to acquire publish entry lock")}},API:it,AuthenticationPage:re}})(),c.DecapCmsBackendGithub})());
